...
export default function Home() {
   ...
   const fetchedUrl = "https://unsplash.com/photos/mountain-covering-with-clouds-nN1HSDtKdlw";
...
<header className='w-full max-w-screen-xl mx-auto bg-white md:py-4'>
     <nav
        className={"items-center justify-between w-full py-4   md:py-0  px-4 text-lg text-gray-700 bg-white " + (isMenuHidden ? 'flex' : '')}
    //    className={` ${isMenuHidden ? 'flex      items-center   justify-between w-full   py-4  md:py-0 px-4 text-lg text-gray-700 bg-white ' :
    //     '  '}`}
      ...
      >
       ...
       <div  className={` ${isMenuHidden ? 'hidden-custom' : ''}`}>
  ...
  <div class="flex  h-screen items-center justify-center bg-black-900 p-5 py-32 text-center bgimg"
  ...
  >
    ...
    <div class="grid md:grid-cols-2 grid-cols-1 items-center gap-10 md:px-10">
      ...
      <div>
        <h1 class="mb-2 text-3xl font-bold text-white"><span class="text-green-500">Hi,</span> I'm Abdul Rehman UI Developer</h1>
        ...
        <a href="#product" class="flex w-full items-center justify-center gap-1 rounded-2xl bg-rose-500 p-5 py-3 font-semibold text-white hover:bg-rose-700"  style={{color: "white"}}>
          ...
          <a  href="#service" class="flex w-full items-center text-green-500 justify-center gap-2 rounded-2xl bg-white p-5 py-3 font-semibold">
          Service
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6">
              <path stroke-linecap="round" stroke-linejoin="round" d="m9 12.75 3 3m0 0 3-3m-3 3v-7.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
            </svg>
      ...
      <div className='avatar-profile'>
        <img src="https://avatars.githubusercontent.com/u/25557675?s=400&v=4" alt="" class="md:size-96 size-72 rounded-full " />
      </div>
    </div>
    </div>
  </div>
